...
def run_pingo(item_path: str, lossy: bool) -> Optional[str]:
    """Run pingo on the directory and return its output."""
    if lossy:
        cmd = ["pingo", "-s4", "-webp", "-process=4", item_path]
    else:
        cmd = [
            "pingo",
            "-s4",
...
def process_single_folder(
    item_path: str, zip_file_path: str, lossy: bool, skip_pingo: bool
) -> Optional[str]:
    ...
    pingo_output = None
    if not skip_pingo:
        pingo_output = run_pingo(item_path, lossy)
